...
void ADC_StructInit(ADC_InitTypeDef* ADC_InitStruct)
{
  ...
  ADC_InitStruct->ADC_Mode = ADC_Mode_Single;
	...
	//ADC_InitStruct->ADC_TRGEN = ADC_TRGDIS;
	ADC_InitStruct->ADC_TRGEN = ADC_TRG_Disable;
...
/**
  * @brief  Set the  channels conversion value offset
  * @param ADCx: where x can be 1, 2  to select the ADC peripheral.
  * @param ADC_InjectedChannel: the ADC injected channel to set its
  *   offset. 
  *   This parameter can be one of the following values:
  * @arg ADC_InjectedChannel_0: Injected Channel0 selected
  * @arg ADC_InjectedChannel_1: Injected Channel1 selected
  * @arg ADC_InjectedChannel_2: Injected Channel2 selected
  * @arg ADC_InjectedChannel_3: Injected Channel3 selected
  * @arg ADC_InjectedChannel_4: Injected Channel4 selected
  * @arg ADC_InjectedChannel_5: Injected Channel5 selected
  * @arg ADC_InjectedChannel_6: Injected Channel6 selected
  * @arg ADC_InjectedChannel_7: Injected Channel7 selected
  * @arg ADC_InjectedChannel_8: Injected Channel8 selected
...
uint16_t ADC_GetInjectedConversionValue(ADC_TypeDef* ADCx, uint8_t ADC_InjectedChannel)
{
  ...
  return (uint16_t) (*(__IO uint32_t*) (((*(uint32_t*)&ADCx) + ADC_InjectedChannel)));
}
...
void ADC_TempSensorVrefintCmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    ADC1->ADCHS |= ADCHS_TSVREFE_Set ;
		ADC2->ADCHS |= ADCHS_TSVREFE_Set ;
  }
  else
  {
    /* Disable the temperature sensor and Vrefint channel*/
    ADC1->ADCHS &= ADCHS_TSVREFE_Reset;
		ADC2->ADCHS &= ADCHS_TSVREFE_Reset;
  }
}
