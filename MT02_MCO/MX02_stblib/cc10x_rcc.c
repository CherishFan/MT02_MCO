...
void RCC_PLLDMDNConfig(uint32_t RCC_PLLSource, uint32_t RCC_PLLDN, uint32_t RCC_PLLDM)
{
  ...
  /* Clear PLLDN, PLLDM bits */
	...
	tmpreg0 &= 0x038fffff;
  //tmpreg1 &= CFGR_PLL_Mask;
  /* Set the PLL configuration bits */
	tmpreg0 |= (RCC_PLLDN<<26)|(RCC_PLLDM<<20);
  //tmpreg1 |= RCC_PLLSource;
  /* Store the new value */
  //RCC->CFGR = tmpreg1;
	RCC->CR = tmpreg0;
	RCC->CFGR |= RCC_PLLSource;
}
...
void RCC_PLLConfig(uint32_t RCC_PLLSource, uint32_t RCC_PLLMul)
{
  ...
  RCC->CFGR = tmpreg;
	...
	{
		RCC_PLLDMDNConfig(RCC_PLLSource, 0x00000009, 0x00000001);//Frclk*9/2
	}
}
...
/**
  ...
  *   RCC_APB2Periph_TIM8, RCC_APB2Periph_USART1, RCC_APB2Periph_COMP,
  *   RCC_APB2Periph_ALL
  * @param NewState: new state of the specified peripheral clock.
  *   This parameter can be: ENABLE or DISABLE.
  * @retval : None
