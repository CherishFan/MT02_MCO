...
{
	//while(read_mreg32(USB_EPn_CTRL(EPIN))&0x80);//wait the last transfer finish ????
    switch( CBWCB[0] )
    {
	...
	else 
	{
		...
		if( usb_running_state == USB_IDLE)
		{
			if(usb_send_epn_stall[EPIN])
			{
	            ...
	            uart_printf( "EP Send in stall Cmd\r\n") ;
			}
			else
			{			
...
void usb_hid_report_handle()
{
	...
	{
#ifdef USB_DMA_MODE
		//uint8 hid_val[4]={0x00,0x01,0x01,0x00};//because stack is at bootrom which dam can't access
		uint32 ch;
		...
		//	write_mreg32(USB_EPn_FIFO(EPIN),0x01);//left key pressed
		write_mreg32(USB_EPn_FIFO(EPIN),0x00);//left key not pressed
		write_mreg32(USB_EPn_FIFO(EPIN),1); //x offset two's complement
		write_mreg32(USB_EPn_FIFO(EPIN),1); //x offset two's complement
