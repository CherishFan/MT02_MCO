...
{
    // uart_printf( "SetConfiguration, 0x%bx\n\r", usb_running_ctrl_state ) ;
...
void Chap9_SetAddress( void)	    //0x05
{
	...
	//uart_printf( "SetAddr 0x%lx \n\r",read_mreg32(USB_ADDR)) ;
}
...
//	uint8 temp;
    switch(type)
    {
        ...
        {
        	//uart_printf("configure descript\n");
    ...
    switch( usb_running_ctrl_state )
    {
        ...
        case    USB_CTRL_IN :
        {
			...
			if(wLength>0)
			{
	            ...
	            {
	                write_mreg32(USB_EP0_FIFO,p_ctrl_buf[ctrl_buf_ix]);
					ctrl_buf_ix++;
	            }
	            write_mreg32(USB_EP0_CTRL,count|0x80);
	            wLength -= count;
			}			
			if( wLength == 0)
           	{
