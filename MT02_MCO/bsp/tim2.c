...
{
	if(TIM_GetFlagStatus(TIM2,TIM_FLAG_Update)!= RESET)	 //check UIF位 
	{
		updateFlag = 0x01;
	...
	if(TIM_GetFlagStatus(TIM2,TIM_FLAG_Trigger)!= RESET)	 //check TIF位 
	{
		triggerFlag = 0x01;
		TIM_ClearITPendingBit(TIM2 , TIM_FLAG_Trigger);
	}	

	if(TIM_GetFlagStatus(TIM2,TIM_FLAG_CC1)!= RESET)	 //check capture compare 1位 
	{
		capture1Flag = 0x1;
		TIM_ClearITPendingBit(TIM2 , TIM_FLAG_CC1);
	}	

	if(TIM_GetFlagStatus(TIM2,TIM_FLAG_CC2)!= RESET)	 //check capture compare 2位 
	{
		capture2Flag = 0x1;
		TIM_ClearITPendingBit(TIM2, TIM_FLAG_CC2);
	}	

	if(TIM_GetFlagStatus(TIM2,TIM_FLAG_CC3)!= RESET)	 //check capture compare 2位 
	{
		capture3Flag = 0x01;
		TIM_ClearITPendingBit(TIM2, TIM_FLAG_CC3);
	}

	if(TIM_GetFlagStatus(TIM2,TIM_FLAG_CC4)!= RESET)	 //check capture compare 2位 
	{
		capture4Flag = 0x01;
		TIM_ClearITPendingBit(TIM2, TIM_FLAG_CC4);
	}

	//TIM_PrescalerConfig(TIM2,tim_flag,0x0000);      //分频值,	 TIMx->PSC	TIMx->EGR的bit0 UG 
	//TIM2->RCR = 0x02;  //周期计数次数
...
//设置UG bit，手动产生更新事情
void Tim2_UPCount_test3(TIM_TypeDef* TIMx,uint16_t counter)
{
	...
	TIM_SetClockDivision(TIMx,TIM_CKD_DIV2);  //采样分频	 TIMx->CR1
	...
	TIM_PrescalerConfig(TIMx,0x7,0x0000);      //分频值,	 TIMx->PSC	TIMx->EGR的bit0 UG 
...
//设置UG bit，手动产生更新事情
void Tim2_DOWNCount_test3(TIM_TypeDef* TIMx,uint16_t counter)
{
	NVIC_InitTypeDef         NVIC_InitStructure;
...
//中央对齐模式1
void Tim2_CentAlignMode1_test3(TIM_TypeDef* TIMx,uint16_t counter)
{
...
//中央对齐模式2
void Tim2_CentAlignMode2_test3(TIM_TypeDef* TIMx,uint16_t counter)
{
	NVIC_InitTypeDef         NVIC_InitStructure;
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2, ENABLE);
	TIM_DeInit(TIMx);
...
void Tim2_CentAlignMode3_test3(TIM_TypeDef* TIMx,uint16_t counter)
{
	NVIC_InitTypeDef         NVIC_InitStructure;
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2, ENABLE);
	TIM_DeInit(TIMx);
	...
	TIM_ITConfig(TIMx, TIM_IT_Update, ENABLE);	//TIM2的更新事件中断使能
...
void Tim2_ExtClkMode1_test(TIM_TypeDef* TIMx)
{
...
//外部时钟源模式2 外部触发ETR引脚
void Tim2_ExtClkMode2_test(TIM_TypeDef* TIMx)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2, ENABLE);
	TIM_DeInit(TIMx);
...
//捕获比较通道
void Tim2_capture_compare_test(TIM_TypeDef* TIMx,uint16_t channel,uint16_t polarity,uint16_t selectPin,uint16_t pscclk,uint16_t filter)
{
	uint16_t temp = 0x0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
...
//输入捕获模式
void Tim2_capture_test(TIM_TypeDef* TIMx,uint16_t channel,uint16_t polarity,uint16_t selectPin,uint16_t pscclk,uint16_t filter)
{
...
//tim2的ch3通道  PA2  
	...
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);
	//NVIC1_IRQEnable(TIM2_IRQn);          //Tim2 的终端
  ...
  TIM_ICInitStruct.TIM_ICFilter = filter;
	...
	*/
	DMA_DeInit(DMA1_Channel1);
		DMA_InitStructure.DMA_PeripheralBaseAddr = (TIM2_BASE+0x3c);
...
//PWM输入模式
void Tim2_PWM_capture_test(TIM_TypeDef* TIMx)
{
	uint16_t temp = 0x0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
	...
	TIM_ITConfig(TIMx, TIM_IT_CC2, ENABLE);	//TIM2的触发中断使能
...
//输出比较模式
void Tim2_compare_test(TIM_TypeDef* TIMx,uint16_t channel)
{
	uint16_t temp = 0x0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_OCInitTypeDef TIM_OCInitStruct;
  ...
  */
	...
	DMA_InitStructure.DMA_PeripheralBaseAddr = (TIM2_BASE+0x40);
...
//PWM1 pwm2模式向上  输出
void Tim2_PWM_edgeUp_test(TIM_TypeDef* TIMx,uint16_t channel)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_OCInitTypeDef TIM_OCInitStruct;
	NVIC_InitTypeDef         NVIC_InitStructure;
...
//pwm1 PWM2模式向下计数  输出
void Tim2_PWM_edgeDown_test(TIM_TypeDef* TIMx,uint16_t channel)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_OCInitTypeDef TIM_OCInitStruct;
	NVIC_InitTypeDef         NVIC_InitStructure;
...
void Tim2_PWM_CenterAligned_test(TIM_TypeDef* TIMx,uint16_t channel)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_OCInitTypeDef TIM_OCInitStruct;
	NVIC_InitTypeDef         NVIC_InitStructure;
...
//单脉冲模式
void Tim2_opm_test(TIM_TypeDef* TIMx,uint16_t channel,uint16_t polarity,uint16_t selectPin,uint16_t pscclk,uint16_t filter)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
	TIM_OCInitTypeDef TIM_OCInitStruct;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);  //开启GPIOA端口
	GPIO_InitStructure.GPIO_Pin  =  GPIO_Pin_1;            //PA1 ch2
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; //浮空输入
	GPIO_Init(GPIOA, &GPIO_InitStructure);
...
//在外部事件时清除OCxREF信号
void Tim2_ETR_OCREF_test(TIM_TypeDef* TIMx,uint16_t channel)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_OCInitTypeDef TIM_OCInitStruct;
	NVIC_InitTypeDef         NVIC_InitStructure;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);  //开启GPIOA端口
	GPIO_InitStructure.GPIO_Pin  =  GPIO_Pin_0;            //PA0 etr
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; //浮空输入
	GPIO_Init(GPIOA, &GPIO_InitStructure);
	
	GPIO_InitStructure.GPIO_Pin  =  GPIO_Pin_3;            //PA3 ch4
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP; //复用推免输出
	GPIO_Init(GPIOA, &GPIO_InitStructure);
...
//编码器接口模式
void Tim2_program_test(TIM_TypeDef* TIMx)
{
	uint16_t count = 0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
...
void Tim2_sreset_test(TIM_TypeDef* TIMx)
{
	uint16_t count = 0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
	...
	NVIC_Init(&NVIC_InitStructure);
	//NVIC1_IRQEnable(TIM2_IRQn);          //Tim2 的终端
	TIM_ITConfig(TIMx, TIM_IT_Trigger, ENABLE);	//TIM2的触发中断使能
...
//Tim1定时器和外部触发的  门控模式
void Tim2_gate_test(TIM_TypeDef* TIMx)
{
	uint16_t count = 0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
...
//Tim1定时器和外部触发的  触发模式
void Tim2_trigger_test(TIM_TypeDef* TIMx)
{
	uint16_t count = 0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
...
//外部时钟模式2 + 触发模式
void Tim2_extclk2_trigger_test(TIM_TypeDef* TIMx)
{
	uint16_t count = 0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);  //开启GPIOA端口
	GPIO_InitStructure.GPIO_Pin  =  GPIO_Pin_0|GPIO_Pin_1;   //PA0 ch1 etr PA1 ch2
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; //浮空输入
	GPIO_Init(GPIOA, &GPIO_InitStructure);
