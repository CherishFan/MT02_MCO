...
void TIM4_IRQHandler (void)
{
	...
	if(TIM_GetFlagStatus(TIM4,TIM_FLAG_Update)!= RESET)	 //check UIF位 
	{
		updateFlag = 0x01;
	...
	if(TIM_GetFlagStatus(TIM4,TIM_FLAG_Trigger)!= RESET)	 //check TIF位 
	{
		triggerFlag = 0x01;
		TIM_ClearITPendingBit(TIM4 , TIM_FLAG_Trigger);
	}	

	if(TIM_GetFlagStatus(TIM4,TIM_FLAG_CC1)!= RESET)	 //check capture compare 1位 
	{
		capture1Flag = 0x1;
		TIM_ClearITPendingBit(TIM4 , TIM_FLAG_CC1);
	}	

	if(TIM_GetFlagStatus(TIM4,TIM_FLAG_CC2)!= RESET)	 //check capture compare 2位 
	{
		capture2Flag = 0x1;
		TIM_ClearITPendingBit(TIM4, TIM_FLAG_CC2);
	}	

	if(TIM_GetFlagStatus(TIM4,TIM_FLAG_CC3)!= RESET)	 //check capture compare 2位 
	{
		capture3Flag = 0x01;
		TIM_ClearITPendingBit(TIM4, TIM_FLAG_CC3);
	}

	if(TIM_GetFlagStatus(TIM4,TIM_FLAG_CC4)!= RESET)	 //check capture compare 2位 
	{
		capture4Flag = 0x01;
		TIM_ClearITPendingBit(TIM4, TIM_FLAG_CC4);
	}

	//TIM_PrescalerConfig(TIM4,tim_flag,0x0000);      //分频值,	 TIMx->PSC	TIMx->EGR的bit0 UG 
	//TIM4->RCR = 0x02;  //周期计数次数
 /* 
...
//设置UG bit，手动产生更新事情
void Tim4_UPCount_test3(TIM_TypeDef* TIMx,uint16_t counter)
{
	...
	NVIC_InitStructure.NVIC_IRQChannel = TIM4_IRQn;           //设置中断向量
	...
	TIM_ITConfig(TIMx, TIM_IT_Update, ENABLE);	//Tim4的更新事件中断使能
...
//设置UG bit，手动产生更新事情
void Tim4_DOWNCount_test3(TIM_TypeDef* TIMx,uint16_t counter)
{
	NVIC_InitTypeDef         NVIC_InitStructure;
...
//中央对齐模式1
void Tim4_CentAlignMode1_test3(TIM_TypeDef* TIMx,uint16_t counter)
{
...
//中央对齐模式2
void Tim4_CentAlignMode2_test3(TIM_TypeDef* TIMx,uint16_t counter)
{
	NVIC_InitTypeDef         NVIC_InitStructure;
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);
	TIM_DeInit(TIMx);
...
void Tim4_CentAlignMode3_test3(TIM_TypeDef* TIMx,uint16_t counter)
{
	NVIC_InitTypeDef         NVIC_InitStructure;
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);
	TIM_DeInit(TIMx);
...
void Tim4_ExtClkMode1_test(TIM_TypeDef* TIMx)
{
	...
	TIM_ITConfig(TIMx, TIM_IT_Trigger, ENABLE);	//Tim4的触发中断使能
*/
...
//外部时钟源模式2 外部触发ETR引脚
void Tim4_ExtClkMode2_test(TIM_TypeDef* TIMx)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);
	TIM_DeInit(TIMx);
...
//捕获比较通道
void Tim4_capture_compare_test(TIM_TypeDef* TIMx,uint16_t channel,uint16_t polarity,uint16_t selectPin,uint16_t pscclk,uint16_t filter)
{
	uint16_t temp = 0x0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
...
//输入捕获模式
void Tim4_capture_test(TIM_TypeDef* TIMx,uint16_t channel,uint16_t polarity,uint16_t selectPin,uint16_t pscclk,uint16_t filter)
{
...
//tim3的ch3通道  PB0  
	...
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);
	//NVIC1_IRQEnable(TIM4_IRQn);          //Tim4 的终端
  ...
  TIM_ICInitStruct.TIM_ICFilter = filter;
	...
	*/
	DMA_DeInit(DMA1_Channel1);
		DMA_InitStructure.DMA_PeripheralBaseAddr = (TIM4_BASE+0x3c);
...
//PWM输入模式
void Tim4_PWM_capture_test(TIM_TypeDef* TIMx)
{
	uint16_t temp = 0x0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
	...
	NVIC_Init(&NVIC_InitStructure);
	//NVIC1_IRQEnable(TIM4_IRQn);          //Tim4 的终端
	TIM_ITConfig(TIMx, TIM_IT_CC1, ENABLE);	//Tim4的触发中断使能
	TIM_ITConfig(TIMx, TIM_IT_CC2, ENABLE);	//TIM4的触发中断使能
...
//输出比较模式
void Tim4_compare_test(TIM_TypeDef* TIMx,uint16_t channel)
{
	uint16_t temp = 0x0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_OCInitTypeDef TIM_OCInitStruct;
  NVIC_InitTypeDef         NVIC_InitStructure;
	...
	NVIC_Init(&NVIC_InitStructure);
	//NVIC1_IRQEnable(TIM4_IRQn);          //Tim4 的终端
	TIM_ITConfig(TIMx, TIM_IT_CC4, ENABLE);	//Tim4的触发中断使能
  ...
  */
	...
	DMA_InitStructure.DMA_PeripheralBaseAddr = (TIM4_BASE+0x40);
...
//PWM1 pwm2模式向上  输出
void Tim4_PWM_edgeUp_test(TIM_TypeDef* TIMx,uint16_t channel)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_OCInitTypeDef TIM_OCInitStruct;
	NVIC_InitTypeDef         NVIC_InitStructure;
	...
	TIM_OCInitStruct.TIM_OCMode = TIM_OCMode_PWM2; // 值匹配时  OCxREF 的状态  PWM1 模式
  	TIM_OCInitStruct.TIM_OutputState = TIM_OutputState_Enable;
  	TIM_OCInitStruct.TIM_OutputNState = TIM_OutputNState_Disable;
  	TIM_OCInitStruct.TIM_Pulse = 0x004;    //0x04频率1.667MHz 600ns 不变PWM2模式下，随着TIM PULSE的值改变，脉宽改变，值越小，high越宽。PWM1模式与PWm2相反 
...
//pwm1 PWM2模式向下计数  输出
void Tim4_PWM_edgeDown_test(TIM_TypeDef* TIMx,uint16_t channel)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_OCInitTypeDef TIM_OCInitStruct;
	NVIC_InitTypeDef         NVIC_InitStructure;
	...
	NVIC_Init(&NVIC_InitStructure);
	//NVIC1_IRQEnable(TIM4_IRQn);          //Tim4 的终端
	TIM_ITConfig(TIMx, TIM_IT_CC3, ENABLE);	//Tim4的触发中断使能
...
void Tim4_PWM_CenterAligned_test(TIM_TypeDef* TIMx,uint16_t channel)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_OCInitTypeDef TIM_OCInitStruct;
	NVIC_InitTypeDef         NVIC_InitStructure;
	...
	TIM_OCInitStruct.TIM_OCMode = TIM_OCMode_PWM1; // 值匹配时  OCxREF 的状态  PWM1 模式
  	TIM_OCInitStruct.TIM_OutputState = TIM_OutputState_Enable;
  	TIM_OCInitStruct.TIM_OutputNState = TIM_OutputNState_Disable;
  	TIM_OCInitStruct.TIM_Pulse = 0x0007;          //8.6us 119KHz 模式1、2、3下，值越大，high越宽
  	TIM_OCInitStruct.TIM_OCPolarity = TIM_OCPolarity_High;
  	TIM_OCInitStruct.TIM_OCNPolarity = TIM_OCPolarity_High;
  	TIM_OCInitStruct.TIM_OCIdleState = TIM_OCIdleState_Reset;
  	TIM_OCInitStruct.TIM_OCNIdleState = TIM_OCNIdleState_Reset;
...
//单脉冲模式
void Tim4_opm_test(TIM_TypeDef* TIMx,uint16_t channel,uint16_t polarity,uint16_t selectPin,uint16_t pscclk,uint16_t filter)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
	TIM_OCInitTypeDef TIM_OCInitStruct;
...
//在外部事件时清除OCxREF信号
void Tim4_ETR_OCREF_test(TIM_TypeDef* TIMx,uint16_t channel)
{
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_OCInitTypeDef TIM_OCInitStruct;
	NVIC_InitTypeDef         NVIC_InitStructure;
...
//编码器接口模式
void Tim4_program_test(TIM_TypeDef* TIMx)
{
	uint16_t count = 0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);  //开启GPIOA端口
	GPIO_InitStructure.GPIO_Pin  =  GPIO_Pin_6;            //PA0 ch1
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; //浮空输入
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	...
	TIM_ITConfig(TIMx, TIM_IT_CC2, ENABLE);	//Tim4的触发中断使能
	TIM_ITConfig(TIMx, TIM_IT_Trigger, ENABLE);	//Tim4的触发中断使能
	TIM_ITConfig(TIMx, TIM_IT_Update, ENABLE);	//Tim4的触发中断使能
...
void Tim4_sreset_test(TIM_TypeDef* TIMx)
{
	uint16_t count = 0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
...
//Tim1定时器和外部触发的  门控模式
void Tim4_gate_test(TIM_TypeDef* TIMx)
{
	uint16_t count = 0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
...
//Tim1定时器和外部触发的  触发模式
void Tim4_trigger_test(TIM_TypeDef* TIMx)
{
	uint16_t count = 0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB, ENABLE);  //开启GPIOA端口
	GPIO_InitStructure.GPIO_Pin  =  GPIO_Pin_7;            //PB7 ch2
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; //浮空输入
	GPIO_Init(GPIOB, &GPIO_InitStructure);
...
//外部时钟模式2 + 触发模式
void Tim4_extclk2_trigger_test(TIM_TypeDef* TIMx)
{
	uint16_t count = 0;
	GPIO_InitTypeDef GPIO_InitStructure;//定义GPIO初始化结构体变量
	TIM_ICInitTypeDef TIM_ICInitStruct;
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOD, ENABLE);  //开启GPIOA端口
	GPIO_InitStructure.GPIO_Pin  =  GPIO_Pin_0;            //PA0 ch1
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING; //浮空输入
